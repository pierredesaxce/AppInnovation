...
class LSTMLight(nn.Module):
    ...
    def forward(self, input, hidden):
        _, (hidden, _) = self.lstm(input.unsqueeze(0), (hidden.to(device), hidden.to(device)))
...
def training(n_epochs, lines):
    ...
    for iter in range(1, n_epochs + 1):
        total_loss = 0
        output, loss = train(inputTensor(lines[iter]), targetTensor(lines[iter])) #modif pour rendre comprendre plus facilement
        total_loss += loss
...
def generate_passwords(decoder, num_samples, max_length, output_file):
    print()
    print('----------------')
    print('|   GENERATE   |')
    print('----------------')
    ...
    all_letters_custom = """0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyz"""

    with open(output_file, 'w') as file:
        for i in range(1, num_samples + 1):
            nc = random.randint(1, 6) # longeur de la seed entre 1 et (max_length // 2) - 1

            starting_letters = "".join(random.choice(all_letters_custom) for _ in range(nc))
            predicted = sample(decoder, starting_letters)

            if predicted not in generated_passwords:
                generated_passwords.add(predicted)
                file.write(predicted + '\n')

            progress(total=num_samples, acc=len(generated_passwords), start=start, epoch=i, l=num_samples)

    print(f'\nGenerated {len(generated_passwords)} unique passwords in {time.time() - start:.2f} seconds.')

def testing_from_file(file1, file2):
    print()
    print('--------------')
    print('|   TEST F   |')
    print('--------------')
    ...
    with open(file1, 'r') as f1:
        passwords1 = set(line.strip() for line in f1)

    with open(file2, 'r') as f2:
        passwords2 = set(line.strip() for line in f2)

    common_passwords = passwords1.intersection(passwords2)
    proportion = len(common_passwords) / len(passwords1)

    print(f'Proportion of passwords from {file1} found in {file2}: {proportion:.2%}')
    print(f'Number of passwords found: {len(common_passwords)}')
    ...
    if nb_samples > 0:
        for i in range(1, nb_samples + 1):
            ...
            while predicted in predicted_current:
                starting_letters = "".join(random.choice(all_letters) for _ in range(nc))
                starting_letters = starting_letters[:random.randint(1, 3)]
                predicted = sample(decoder, starting_letters)
            predicted_current.add(predicted)  # Utiliser add() pour ajouter à l'ensemble

            if predicted in lineTest:
                accuracy += 1
                print(starting_letters, ' -> ', predicted, ' accuracy: ', accuracy, ' / ', i, ' = ',
                      (100 * accuracy / i), ' % and testing corpus contains ', len(lineTest), ' names => coverage of ',
                      (100 * accuracy / len(lineTest)), ' %')
...
if __name__ == '__main__':
    ...
    lineEval = getLines(filenameEval)
    ...
    decoder = LSTMLight(n_letters, 128, n_letters).to(
    ...
    #########
        ...
        decoder = torch.load(args.model)
        decoder.eval().to(device)
        testing(decoder, args.n, lineEval, args.percent)
    #########
    # TEST F#
    #########
    elif args.trainEval == 'testf':
        testing_from_file('generated_passwords_10k_GRU.txt', 'data/Ashley-MadisonTest.txt')
    #########
    # GEN   #
    #########
    elif args.trainEval == 'gen':
        decoder.eval()
        decoder = torch.load(args.model)
        decoder.eval().to(device)
        generate_passwords(decoder, num_samples=10000, max_length=max_length, output_file='generated_passwords_10k_LSTM_V2.txt')
    else:
