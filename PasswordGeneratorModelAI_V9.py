...
def generate_password(model, char_indices, indices_char, max_len, temperature=1.0):
    start_index = np.random.randint(0, len(chars) - max_len - 1)
    generated_password = ""

    sequence = np.zeros((1, max_len, len(chars)))
    for i in range(max_len):
        sequence[0, i, start_index + i] = 1.0

    for i in range(max_len):
        predictions = model.predict(sequence, verbose=0)[0][i]
        next_index = sample(predictions, temperature)
        next_char = indices_char[next_index]
        generated_password += next_char

        sequence[0, i + 1, next_index] = 1.0
...
# Utilisez cette fonction après l'entraînement pour générer des mots de passe
generated_password = generate_password(model, char_indices, indices_char, max_len)
